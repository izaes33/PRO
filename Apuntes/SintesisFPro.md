...
- **Patrón MVC (Model-View-Controller):** - **Model (model):** Clases POJO que representan los datos puros. - **View (view / Main):** Representa la interfaz gráfica o menús de consola. - **Controller (controller):** Gestiona la lógica principal de la aplicación. - **DAO:** aísla por completo las ejecuciones CRUD sobre la base de datos. - **Otros paquetes habituales:** Se suelen añadir paquetes como exceptions (para errores personalizados), utils (herramientas misceláneas), database (conexiones y DAO) y environments (variables de entorno).  
  <br>
